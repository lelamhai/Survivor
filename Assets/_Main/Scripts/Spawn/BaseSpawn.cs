...
public abstract class BaseSpawn : BaseMonoBehaviour
{
    ...
    public void SpawnGameObject(string name, Vector3 point)
    {
        if (_baseHolders.CountPool(name) > 0)
        {
            ...
            SetInitGameObject(undoGameObject, name, point);
        ...
        } else
        {
            ...
            SetInitGameObject(cloneGameObject, name, point);
        }
    }

    private void SetInitGameObject(Transform gameObject, string name, Vector3 point)
    {
